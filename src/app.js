...
class Superwar {
  ...
  fight = () => {
    ...
    let scoreBoard = this.calculateScore();
  ...
  updateScore(){
    let leaderBoard = {hero:0,villain:0,draw:0};
    let winner = this.checkWin()
    if(winner == 'hero'){
      leaderBoard['hero']+=1;
    }
    else if(winner == 'villain'){
      leaderBoard['villain']+=1;
    }
    else{
      leaderBoard['draw']+=1;
    }
  ...
  if(this.totalStrength('hero') > this.totalStrength('villain')){
  return 'hero'}
  
  else if(this.totalStrength('hero') < this.totalStrength('villain'))
  {return 'villain'}
  else return 'endure'
  };
  ...
  this.players.forEach((player) => 
    {
      if (player.type == type) strength += player.strength;
    });
    return strength;
  };
